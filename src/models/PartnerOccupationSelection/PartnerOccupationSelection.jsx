...
const PartnerOccupationSelection = ({
  show,
  onClose,
  onSelect,
  setEducationCareer,
  ...
  useEffect(() => {
    if (show) {
      setTempSelected(educationCareer?.occupation || []);
      setLimitMessage("");
    }
  ...
  const toggleSelect = (item) => {
    setTempSelected((prev) => {
      if (!prev.includes(item) && prev.length >= 10) {
        setLimitMessage("You can select up to 10 occupations.");
        return prev;
      }
        ...
        <div className="selection-wrapper">
          ...
          {!showOtherInput ? (
            <div className="selection-footer">
              ...
              <button className="footer-btn" onClick={handleDone}>
                Done
              </button>
            </div>
          ) : (
...
export default PartnerOccupationSelection;
